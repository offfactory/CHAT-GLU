    ...
    <div class="app-shell ${state.sidebarOpen ? '' : 'sidebar-collapsed'}">
      <aside class="sidebar">
        ...
        <div class="sidebar-bottom">
          ...
          <button class="theme-row" data-action="theme">${state.theme === 'dark' ? icons.sun : icons.moon}<span>Switch to ${state.theme === 'dark' ? 'light' : 'dark'} mode</span></button>
        </div>
      </aside>
      <main class="main">
...
function bindEvents() {
  ...
  app.querySelectorAll('[data-action="roblox-connect"]').forEach((button) => button.addEventListener('click', () => { openSettings(); }));
  ...
  app.querySelector('[data-action="theme"]')?.addEventListener('click', () => { state.theme = state.theme === 'dark' ? 'light' : 'dark'; save(); render(); });
